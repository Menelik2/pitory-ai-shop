...
const Index = () => {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(product => product.name.toLowerCase().includes(searchQuery.toLowerCase()) || product.brand.toLowerCase().includes(searchQuery.toLowerCase()) || product.cpu.toLowerCase().includes(searchQuery.toLowerCase()) || product.generation.toLowerCase().includes(searchQuery.toLowerCase()) || product.ram.toLowerCase().includes(searchQuery.toLowerCase()) || product.storage.toLowerCase().includes(searchQuery.toLowerCase()));
    }
      ...
      <main>
        ...
        <section className="py-16 px-4">
          <div className="container mx-auto">
            <ProductFilters searchQuery={searchQuery} selectedCategory={selectedCategory} sortBy={sortBy} onSearchChange={setSearchQuery} onCategoryChange={setSelectedCategory} onSortChange={setSortBy} />
            <h2 className="text-3xl font-bold text-center mb-12 text-gradient text-amber-500">Featured Products</h2>
            {loading ? <div className="text-center py-12">
                ...
                {!loading && filteredAndSortedProducts.length === 0 && <div className="text-center py-12">
